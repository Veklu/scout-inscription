...
class Scout_Public_Verify {
    ...
    public function render(): string {
        ...
        if (empty($ref) || empty($tok)) {
            ...
            echo '<h2>⚠️ Paramètres manquants</h2>';
            echo '<p>Ce lien ne contient pas les paramètres nécessaires. Veuillez scanner le code QR fourni avec l\'inscription.</p>';
            echo '</div>';
        } else {
    ...
    private function process_verification(string $ref, string $tok): void {
        ...
        if (!Scout_Access_Log::check_rate_limit($ip)) {
            ...
            echo '<div class="scout-verify-box scout-verify-error">';
            echo '<h2>🚫 Trop de tentatives</h2>';
            echo '<p>Vous avez atteint la limite de vérifications. Réessayez dans une heure.</p>';
            echo '</div>';
            return;
        }
        ...
        if (!Scout_Inscription_Model::verify_token($ref, $tok)) {
            ...
            echo '<div class="scout-verify-box scout-verify-error">';
            echo '<h2>🚫 Code QR invalide</h2>';
            echo '<p>Ce code QR ne correspond à aucune inscription valide. Il a peut-être expiré ou été altéré.</p>';
            echo '</div>';
            return;
        }
        ...
        if (!$inscription) {
            echo '<div class="scout-verify-box scout-verify-error">';
            echo '<h2>Inscription introuvable</h2>';
            echo '</div>';
            return;
        }
        ...
        echo '<h2>✅ Inscription vérifiée</h2>';
        ...
        echo '<p>Né(e) le ' . esc_html($inscription->enfant_ddn) . '</p>';
        echo '</div>';
        ...
        if ($inscription->payment_total > 0) {
            ...
            echo '<p>Reçu : ' . number_format($inscription->payment_received, 2) . ' $ / ' . number_format($inscription->payment_total, 2) . ' $</p>';
            if ($balance > 0) {
                echo '<p style="color:#c0392b"><strong>Solde : ' . number_format($balance, 2) . ' $</strong></p>';
            }
        }
        echo '</div>';
        ...
        if (!empty($allergies)) {
            echo '<p style="color:#c0392b"><strong>⚠️ Allergies :</strong> ' . esc_html(implode(', ', $allergies)) . '</p>';
        } else {
            echo '<p>Aucune allergie déclarée</p>';
        }
        if (!empty($medical['medicaments'])) {
            echo '<p><strong>Médicaments :</strong> ' . esc_html($medical['medicaments']) . '</p>';
        }
        if (($medical['attention_particuliere'] ?? 'non') !== 'non') {
            echo '<p><strong>Attention :</strong> ' . esc_html($medical['attention_detail'] ?? '') . '</p>';
        }
        echo '</div>';
        ...
        if (is_user_logged_in() && (current_user_can('scout_view_inscriptions') || current_user_can('manage_options'))) {
            ...
            echo '<h3 style="margin:0 0 12px;color:#007748">🖨️ Zone administrateur — Réimprimer pour un parent</h3>';
            ...
            echo '<button onclick="reprintConfirmation()" style="padding:8px 16px;background:none;border:2px solid #007748;color:#007748;border-radius:6px;font-size:13px;font-weight:600;cursor:pointer">🖨️ Imprimer la confirmation avec QR</button>';
            ...
            echo '<p style="font-size:11px;color:#6a6a62;margin-top:12px">Cet accès est enregistré dans le journal (Loi 25).</p>';
            echo '</div>';
        }
